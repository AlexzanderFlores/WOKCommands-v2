...
class Cooldowns {
    ...
    constructor(instance, cooldownConfig) {
        const { errorMessage = 'Please wait {TIME} before doing that again.', botOwnersBypass = false, dbRequired = 300, // 5 minutes
         } = cooldownConfig;
    ...
    getKey(cooldownType, userId, actionId, guildId) {
        const isPerUser = cooldownType === exports.cooldownTypes[0];
        const isPerUserPerGuild = cooldownType === exports.cooldownTypes[1];
        const isPerGuild = cooldownType === exports.cooldownTypes[2];
        const isGlobal = cooldownType === exports.cooldownTypes[3];
    ...
    async start(cooldownUsage) {
        ...
        if (!exports.cooldownTypes.includes(cooldownType)) {
            throw new Error(`Invalid cooldown type "${cooldownType}". Please use one of the following: ${exports.cooldownTypes}`);
        }
